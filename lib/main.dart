...
Future<void> setupFirebasePushNotifications() async {
  try {
    ...
    // Subscribe to the 'all' topic for general notifications
    await messaging.subscribeToTopic('all');
    debugPrint("📢 Subscribed to Firebase topic: all");

    // Optional: Log the initial token
    final token = await messaging.getToken();
    debugPrint('🔥 Initial FCM Device Token: $token');

    // --- 👇 NEW CODE: LISTEN FOR TOKEN REFRESHES 👇 ---
    // This stream fires when a new token is generated by Firebase.
    FirebaseMessaging.instance.onTokenRefresh.listen((newFcmToken) async {
      ...
      // When the token refreshes, we need to send the new one to our server.
      // We check SharedPreferences to see if a user is logged in.
      final prefs = await SharedPreferences.getInstance();
      final accessToken = prefs.getString('accessToken');

      if (accessToken != null) {
        // If logged in, send the new token.
    ...
    FirebaseMessaging.onMessage.listen((RemoteMessage message) {
      // ... (your existing foreground handler is fine)
      debugPrint(
    ...
    FirebaseMessaging.onMessageOpenedApp.listen((RemoteMessage message) {
      // ... (your existing background tap handler is fine)
      debugPrint(
