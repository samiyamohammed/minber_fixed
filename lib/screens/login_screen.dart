...
//   }
// }
// lib/screens/login_screen.dart (Fully Corrected & Ready to Paste)
...
import '../core/app_colors.dart';
import '../providers/user_provider.dart'; // ✅ 1. IMPORT THE USER PROVIDER
...
class _LoginPageState extends State<LoginPage> {
  ...
  // ✅ 2. NEW HELPER TO MANAGE "REMEMBER ME" CREDENTIALS
  // This is a UI concern, so it stays in the LoginPage.
  ...
  // ✅ 3. THE FULLY REFACtORED LOGIN METHOD
  Future<void> _login() async {
    if (!_formKey.currentState!.validate()) {
      _logger.w("Form validation failed.");
      return;
    ...
    // Get the provider instance (listen: false is crucial in functions)
    final userProvider = Provider.of<UserProvider>(context, listen: false);

    try {
      // Delegate the API call and state management to the provider
      ...
      if (mounted) {
        if (loginSuccess) {
          // If login was successful, handle "Remember Me" and navigate
          await _handleRememberMe();
          _showToast("Login Successful 🎉", bgColor: Colors.green);
          Navigator.pushReplacementNamed(context, '/home');
        } else {
          // The provider already logged the detailed error. We just show a generic message.
          _showToast("❌ Invalid email or password.");
        }
      }
    } catch (e) {
      // Catch any unexpected errors from the provider call
  ...
  // ✅ 4. Your build method and other UI logic remain completely unchanged!
  @override
  Widget build(BuildContext context) {
    final size = MediaQuery.of(context).size;
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton(
                      onPressed: _isLoading
                          ? null
                          : _login, // This now calls the refactored method
                      style: ElevatedButton.styleFrom(
                        backgroundColor: AppColors.primary,
                        foregroundColor: Colors.white,
                        shape: RoundedRectangleBorder(
